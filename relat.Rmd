...
```{r}
df_boost <- boost_tree(trees = 1000,
                        tree_depth = tune(),
                        min_n = tune(),
                        learn_rate = tune(),
                        sample_size = tune(),
...
df_boost_glm <- logistic_reg(penalty = tune(), mixture = 1) %>% 
  set_engine("glmnet")

df_boost_rf = rand_forest(mode = 'classification', 
                          trees = 1000,
                          mtry = tune(),
                          min_n = tune()) |> 
  set_engine('ranger', importance = "impurity")
...
tune_results <- tune_grid(
    ...
    grid       = xgb_grid,#15
    metrics    = yardstick::metric_set(roc_auc, spec, sens), 
    control    = control_grid(save_pred = TRUE)
)

#RF

tune_work_rf <- workflow() %>%
    add_model(df_boost_rf) %>%
    add_recipe(df_rec1)
tune_results_rf <- tune_grid(
    object     = tune_work_rf,
    resamples  = df_vf,
    param_info = parameters(tune_work_rf),
    grid       = 15, 
    metrics    = yardstick::metric_set(roc_auc, spec, sens), 
    control    = control_grid(save_pred = TRUE)
)
...
tune_work_glm <- workflow() %>%
    add_model(df_boost_glm) %>%
    add_recipe(df_rec1)
tune_results_glm <- tune_grid(
    object     = tune_work_glm,
    resamples  = df_vf,
    param_info = parameters(tune_work_glm),
    grid       = 15, 
    metrics    = yardstick::metric_set(roc_auc, spec, sens), 
    control    = control_grid(save_pred = TRUE)
)
...
show_best(tune_results, metric = "spec") #[3,]
```


```{r}
tuned_best <- select_best(tune_results, metric = "spec") #[3,]
fin_wflw <- tune_work  %>%
...
```{r}
tuned_best_rf <- select_best(tune_results_rf, metric = "spec") 
fin_wflw_rf <- tune_work_rf  %>%
    finalize_workflow(parameters = tuned_best_rf)
wflw_fit_rf <- fin_wflw_rf %>%
    fit(df_train)
...
```{r}
tuned_best_glm <- select_best(tune_results_glm, metric = "roc_auc") 
fin_wflw_glm <- tune_work_glm  %>%
    finalize_workflow(parameters = tuned_best_glm)
wflw_fit_glm <- fin_wflw_glm %>%
    fit(df_train)
...
$$H_0:\beta_i = 0\text{ (Covariável nula)}$$
...
estimate <- fit1 %>%
  ...
  tidy() |> with(estimate)
```
A Tabela \@ref(tab:est) detalha, algumas estatísticas muito importantes sobre as covariáveis, mas principalmente informa que todas as variáveis preditivas são significativo a 5% e tem um p-valor << 0.001.
