...
graph<-function(data,l){
  data %>% 
    as_tibble() %>% 
      ggplot(aes(as.numeric(row.names(data  %>% as_tibble())),value))+
      geom_point(aes(colour = as.factor(df_dec$Legendary)))+
...
```{r g1, fig.cap = "Frequência do total de pokémons por geração"}
df_dec |> 
  group_by(Generation) |> 
  count() |> 
...
```
Com a Figura \@ref(fig:g1), podemos notar uma "sazonalidade" para o desenvolvimento de pokémons, visto que as gerações ímpares se sobresãem em número com todas com pelo menos 160 pokémons novos e para as gerações pares tem-se uma queda notável, com um mínimo 82 novos pokémons na geração 6 e um máximo de 121 na geração 5.

```{r g2, fig.cap = "Frequência do total de pokémons lendários por geração"}
df_dec |> 
  filter(Legendary == "Sim") |> 
  group_by(Generation) |> 
  count() |> 
...
Antagônico a Figura \@ref(fig:g1), na Figura \@ref(fig:g2) a terceira e quinta gerações se evidenciam, com a criação de pokémons lendários.
...
df_boostado <- boost_tree(trees = 1000,
                        tree_depth = 7,
                        min_n = 6,
                        learn_rate = 0.07464001,
                        sample_size = 0.8941112,
                        mtry = 2,
                        loss_reduction = 1.879964e-06
                       ) |>
  set_engine('xgboost') |> 
...
df_work = workflow_set(list(si=df_rec1),
                     list(logistic=df_rl,rf=df_rf, xg = df_xgb, df_boostado), cross=T)
```
```{r}
set.seed(42)
...
```{r}
fit4 = df_work %>% 
  extract_workflow("si_boost_tree_4") %>% 
  fit(data = df_train)
```
...
```{r}
cook1(fit1, "Distância do Modelo 1") + ggrepel::geom_text_repel(aes(label=1:800),size=2.8,point.padding = 0.3)
```
...
```{r}
dffts1(fit1, "Dffits do Modelo 1")+ ggrepel::geom_text_repel(aes(label=1:800),size=2.8,point.padding = 0.3)
```
...
show_best(tune_results, metric = "sens") #[3,]


tuned_best <- select_best(tune_results, metric = "roc_auc") #[3,]
tuned_best2 <-tune_results %>%
    select_best("sens")
fin_wflw <- tune_work  %>%
    finalize_workflow(parameters = tuned_best)
fin_wflw1 <- tune_work  %>%
    finalize_workflow(parameters = tuned_best2)
wflw_fit <- fin_wflw %>%
    fit(df_train)
wflw_fit1 <- fin_wflw1 %>%
    fit(df_train) 
...
#tune_results |>
#  collect_metrics() |>
#  filter(.metric == "sens") |> arrange(desc(mean)) |> View()
#tune_results |>
#  collect_metrics() |>
#  filter(.metric == "spec") |> arrange(desc(mean)) |> View()
```
