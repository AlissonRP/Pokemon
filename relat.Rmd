...
## Random Forest 
Árvores de decisão são modelos que já existem a um certo tempo, apesar de terem uma grande vantagem em interpretabilidade são fracas em termos preditivos, assim a  idéia de Random Forest é combinar diversas árvores alterando o conjunto de treinamento de cada uma elas para gerar diversidade na predição, pois evidentemente queremos aprender padrões nos dados.
...
Vamos nessa seção avaliar a existência de pontos influentes
...
# Bibliography
```{r}
cor = function(cor = 'black'){
iris |> 
  ggplot(aes(Sepal.Length, Sepal.Width, color = {{cor}}))+
  geom_point()
}
```
