...
```{r}
tuned_best <- select_best(tune_results, metric = "sens") #[3,]
fin_fit4 <- tune_work  %>%
...
```{r}
tuned_best_rf <- select_best(tune_results_rf, metric = "sens") 
fin_fit5 <- tune_work_rf  %>%
...
fit5 |> 
  ...
  rename(Predição = .pred_class) |> 
  mypdf1::pdf1_tbl_freq2(Legendary , Predição, 'Matriz de confusão para os dados de teste no modelo 
                         de Random Forest Tunnado')

```


```{r}
tuned_best_glm <- select_best(tune_results_glm, metric = "sens") 
fin_fit6 <- tune_work_glm  %>%
